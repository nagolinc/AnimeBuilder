...
def get_img_pose(img_path,processor):
    

    tmp_dir = "samples/tmp_dir"
    os.makedirs((tmp_dir),exist_ok=True)

    print("ABOUT TO DIE",[processor, "-f", img_path, "-out_dir", tmp_dir, "-pose"])

    subprocess.call([processor, "-f", img_path, "-out_dir", tmp_dir, "-pose"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
...
def parse_phoneme_file(phoneme_path,use_index = True,phindex_location = "phindex.json"):
...
def get_pose_from_audio(img,audio,audio2pose):
    ...
    #load from globals if they are not none
    global _ckpt_para
    global _generator

    if _ckpt_para is None:
        ckpt_para = torch.load(audio2pose)
        generator.load_state_dict(ckpt_para["generator"])        
        generator.eval()
        _ckpt_para = ckpt_para
        _generator = generator
    else:
        generator = _generator
        ckpt_para = _ckpt_para
