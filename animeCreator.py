...
class AnimeBuilder:
    ...
    def __init__(self,diffusionModel="hakurei/waifu-diffusion"):

        print("LOADING TEXT MODEL")

        # text model
        self.textModel = 'EleutherAI/gpt-neo-1.3B'

        self.textGenerator = pipeline('text-generation',
    ...
    def doGen(self,prompt, num_inference_steps=30):
        image = self.pipe([prompt],
                    negative_prompt=["text, watermark, lowres, bad anatomy, bad hands, text, error, missing fingers, cropped, worst quality, low quality, normal quality, jpeg artifacts, watermark, blurry, grayscale, deformed weapons, deformed face, deformed human body"],
                    guidance_scale=7.5,
    ...
    def descriptionToCharacter(self,description):
        thisObject = WorldObject(templates, self.textGenerator, "descriptionToCharacter", objects={
    ...
    def advanceStory(self,story, character1=None, character2=None, genTextAmount=20):
        ...
        newStory = WorldObject(templates, self.textGenerator, "advancePlot", objects={
    ...
    def sceneToTranscript(self,scene, k=3, character1=None, character2=None):
        ...
        thisObject = WorldObject(templates, self.textGenerator,
                                "sceneToTranscript", objects)
        return thisObject


    def watchAnime(self,synopsis=None, subplot1=None, scene1=None, character1=None, k=100, amt=25, promptSuffix="", portrait_size=128):
        objects = {}
        if synopsis:
        ...
        plotOverview = WorldObject(
            templates, self.textGenerator, "plot overview", genTextAmount=amt, objects=objects)

        subplot = plotOverview.getProperty("part 1")
        objects["subplot"] = subplot

        story = WorldObject(templates, self.textGenerator,
        ...
        for thisCharacter in [character]+supportingCharacters:
            name = thisCharacter.getProperty("name")
        ...
        for i, scene in enumerate(scenes):
            ...
            image = self.doGen(prompt)
            yield {"music": audio}
            yield {"image": image}
            transcript = self.sceneToTranscript(story, k=i+1, character2=character2)
